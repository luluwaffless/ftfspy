export default {
    ...
    testupd: (displayName, placeId, updatedTime, testUpdPing) => `# \`🚨\` [${displayName.toUpperCase()}](<https://www.roblox.com/games/${placeId}>) UPDATED\n-# ${updatedTime}${updatedTime != "now" ? " ago" : ""}\n-# current update probability: ${probability}\n-# ||<@&${testUpdPing}>||`,
    ...
    joinedgame: (preDisplay, displayName, userId, lastLocation, placeId, lastStatus, lastStatusText, lastStatusBegin, probability, statusPing) => `\`🟢\` ${preDisplay} [${displayName}](<https://www.roblox.com/users/${userId}>) is playing [${lastLocation}](https://www.roblox.com/games/${placeId})${lastStatus > 0 ? `\n-# was ${lastStatus == 2 ? `playing ${lastLocation}` : lastStatusText} for ${lastStatusBegin}` : ""}\n-# current update probability: ${probability}\n-# ||<@&${statusPing}>||`,
    changedstatus: (statusEmoji, status, preDisplay, displayName, userId, lastStatus, lastStatusText, lastStatusBegin, presenceType, probability, studioPing, statusPing) => `\`${statusEmoji}\` ${preDisplay} [${displayName}](<https://www.roblox.com/users/${userId}>) is ${status}${lastStatus > 0 ? `\n-# was ${lastStatus == 2 ? `playing ${lastStatus}` : lastStatusText} for ${lastStatusBegin}` : ""}\n-# current update probability: ${probability}\n-# ||<@&${presenceType == 3 ? studioPing : statusPing}>||`,
